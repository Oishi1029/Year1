...
[ET0, ET1, ET2, ET3, ET4] = determineET(tx, ty, tz, rx, ry, rz, rxv, f1);

%To plot variation graph of E-field from p to q
plotVariationGraph(rxv, f1, ET0, ET1, ET2, ET3, ET4);

%To determine E-fields for 2.4GHz operating frequency
[ET0, ET1, ET2, ET3, ET4] = determineET(tx, ty, tz, rx, ry, rz, rxv, f2);

%To plot variation graph of E-field from p to q
plotVariationGraph(rxv, f2, ET0, ET1, ET2, ET3, ET4);


function [ET0, ET1, ET2, ET3, ET4] = determineET(tx, ty, tz, rx, ry, rz, rxv, f)

    tx_img = 0; % x-coordinate of all 
    bz = 2; 
    ...
    d1 = sqrt((bxv1-tx).^2+(by1-ty)^2+(bz-tz)^2); 
    d2 = sqrt((bxv1-bxv2).^2+(by1-by2)^2+(bz-bz)^2); 
    d3 = sqrt((bxv2-rxv).^2+(by2-ry)^2+(bz-rz)^2); 
    dA = sqrt((bxv2-tx).^2+(by2-ty)^2+(bz-tz)^2);
    dB = sqrt((bxv1-rxv).^2+(by1-ry)^2+(bz-rz)^2);
    ...
    d2 = sqrt((bxv1-bxv2).^2+(by1-by2)^2+(bz-bz)^2); % distance between boundary interception point and the receiver
    d3 = sqrt((bxv2-rxv).^2+(by2-ry)^2+(bz-rz)^2); % distance between boundary interception point and the receiver
    dA = sqrt((bxv2-tx).^2+(by2-ty)^2+(bz-tz)^2);
    dB = sqrt((bxv1-rxv).^2+(by1-ry)^2+(bz-rz)^2);
    ...
    d1 = sqrt((bxv1-tx).^2+(by1-ty)^2+(bz-tz)^2); % distance between boundary interception point and the transmitter
    d2 = sqrt((bxv1-bxv2).^2+(by1-by2)^2+(bz-bz)^2); % distance between boundary interception point and the receiver
    d3 = sqrt((bxv2-bxv3).^2+(by2-by3)^2+(bz-bz)^2); % distance between boundary interception point and the receiver
    d4 = sqrt((bxv3-rxv).^2+(by3-ry)^2+(bz-rz)^2);
    dA = sqrt((bxv2-tx).^2+(by2-ty)^2+(bz-tz)^2);
    dB = sqrt((bxv3-bxv1).^2+(by3-by1)^2+(bz-bz)^2);
    dC = sqrt((bxv2-rxv).^2+(by2-ry)^2+(bz-rz)^2);
    ...
    d2 = sqrt((bxv1-bxv2).^2+(by1-by2)^2+(bz-bz)^2); % distance between boundary interception point and the receiver
    d3 = sqrt((bxv2-bxv3).^2+(by2-by3)^2+(bz-bz)^2); % distance between boundary interception point and the receiver
    d4 = sqrt((bxv3-bxv4).^2+(by3-by4)^2+(bz-bz)^2);
    d5 = sqrt((bxv4-rxv).^2+(by4-ry)^2+(bz-rz)^2);
    dA = sqrt((bxv2-tx).^2+(by2-ty)^2+(bz-tz)^2);
    dB = sqrt((bxv3-bxv1).^2+(by3-by1)^2+(bz-bz)^2);
    dC = sqrt((bxv2-bxv4).^2+(by2-by4)^2+(bz-bz)^2);
    dD = sqrt((bxv3-rxv).^2+(by3-ry)^2+(bz-rz)^2);
    ...
    ET2 = 20*log10(abs(Ed + E1_upper + E1_lower + E2_upper + E2_lower));
    ET3 = 20*log10(abs(Ed + E1_upper + E1_lower + E2_upper + E2_lower + E3_upper + E3_lower));
    ET4 = 20*log10(abs(Ed + E1_upper + E1_lower + E2_upper + E2_lower + E3_upper + E3_lower + E4_upper + E4_lower));
end
